...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ==== Botón WhatsApp ====
  const waButtons = document.querySelectorAll(".js-wa");
  waButtons.forEach((btn) => {
    btn.href =
      "https://wa.me/50767819622?text=Hola%20IMPEL%2C%20quiero%20hacer%20una%20consulta.";
  });

  // ==== Mostrar burbuja del botón "Cómo llegar" ====
  const bubble = document.getElementById("howto-bubble");
  if (bubble) {
    setTimeout(() => bubble.classList.add("visible"), 600);
    setTimeout(() => bubble.classList.remove("visible"), 3800);
  }
});
...
document.addEventListener('DOMContentLoaded', () => {
  const asistenciaBtn = document.querySelector('.asistencia');
  if (asistenciaBtn) {
    asistenciaBtn.addEventListener('click', () => {
      console.log('Asistencia Vial → Abriendo Google Maps');
    });
  }
});
